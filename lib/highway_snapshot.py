"""Transport-independent construction of canonical highway chart messages."""
...
class HighwaySnapshotError(Exception):
    """An expected source error that the transport should deliver to its client."""


async def _report_progress(progress: ProgressCallback | None, stage: str | None) -> None:
...
async def stream_highway_snapshot(
    filename: str,
    arrangement: int = -1,
    ...
    *,
    ...
    """Resolve one song and emit its canonical messages from song_info to ready.
    ...
    dlc = _get_dlc_dir()
    if not dlc:
        raise HighwaySnapshotError("DLC folder not configured")

    song_path = _resolve_dlc_path(dlc, filename)
    if song_path is None:
        raise HighwaySnapshotError("forbidden")
    if not song_path.exists():
        raise HighwaySnapshotError("File not found")
    ...
    try:
        await _report_progress(progress, "Extracting...")
        ...
        try:
            ...
            await _report_progress(progress, None)

        if not song.arrangements:
            raise HighwaySnapshotError("No arrangements found")
        ...
        if not audio_url and is_loose:
            await _report_progress(progress, "Converting audio...")
        ...
        await emit({"type": "beats", "data": beats})

        # Send sections
        sections = [{"name": s.name, "time": s.start_time} for s in song.sections]
        await emit({"type": "sections", "data": sections})
        ...
        if _tempos_out:
            await emit({"type": "tempos", "data": _tempos_out})
        _time_sigs = (loaded_slop.time_signatures
                      if (is_slop and loaded_slop is not None) else None)
        if _time_sigs:
            await emit({"type": "time_signatures", "data": _time_sigs})
        ...
        await emit({"type": "anchors", "data": anchors})
        ...
        await emit({"type": "chord_templates", "data": templates})
        ...
        if is_slop:
            ...
            for xml_path in sorted_xml:
                try:
                    ...
                    # Only swallow unreadable/malformed XML. Emit failures must
                    # bubble to the transport adapter.
                    log.debug(
