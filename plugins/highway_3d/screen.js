...
(function () {
    ...
    // Module-level scratch objects reused each frame to avoid GC
    // churn from per-frame allocation. Only valid for the duration
    ...
    // fill the camera's projected view at the configured distance,
    // then position it at that view-window center and keep it parallel
    // to the camera. Called whenever the projected visible size changes
    // and to position-track the camera each frame.
    function _bgFitBackdropPlane(state) {
        ...
        // Track camera each frame in camera-local space. The view
        // window can be off-centre; copying the camera's world
        // quaternion keeps the plane parallel instead of tilting it
    ...
    const BG_STYLES = {
            ...
            update(s, bands, dt, t) {
                ...
                if (s.crowd) {
                    ...
                    s.crowd.layers.forEach((layer, i) => {
                        ...
                        // cover-crop, independent of projection fit changes.
        ...
        image: {
            build(scene, settings) {
                ...
                // when the projected visible aspect changes.
            ...
            update(s, bands, dt) {
                if (!s) return;
                // Track camera projection every frame. The helper
                // resizes for projected width/height changes, recrops
                // for projected aspect changes, and repositions the
                // plane at the projected view centre.
        ...
        video: {
            build(scene, settings) {
                ...
                try {
                    ...
                    // cam + distance + fit cache fields power that helper.
                    const state = {
                    ...
                    // projected visible aspect changes.
            ...
            update(s) {
                ...
                // work is keeping the plane fitted to the projected
                // view and camera-locked (handled inside the helper).
                _bgFitBackdropPlane(s);
            },
            teardown(s) {
                if (!s) return;
